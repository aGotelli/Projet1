 ...
 * Description
        ...
        must be initialized, and functions are provided for this. However, ideogically
...
////////////////////////////////////////////////~ INDEX ~\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
  ...
  //  Used to update the robot marker (remember to deprecate)
  [[deprecated("You should not use the visualization marker as the URDF is now available to use")]]
  void UpdateMarker(const geometry_msgs::Pose& robotPosture,
                            visualization_msgs::Marker& robotMarker );


  //  Used to update the path generated by the robot in the simulation
  void UpdatePath(const geometry_msgs::Pose& robotPosture,
...
struct Pose2D {
  ...
  //  published message
  {
    robotMarker.pose.position.x = robotPosture.position.x ;
    robotMarker.pose.position.y = robotPosture.position.y ;


    robotMarker.pose.orientation.w = robotPosture.orientation.w ;
    robotMarker.pose.orientation.x = robotPosture.orientation.x ;
    robotMarker.pose.orientation.y = robotPosture.orientation.y ;
    robotMarker.pose.orientation.z = robotPosture.orientation.z ;
  ...
  void UpdatePath(const geometry_msgs::Pose& robotPosture,
                            visualization_msgs::Marker& generatedPath )
  {
    generatedPath.points.push_back( robotPosture.position );
  }
